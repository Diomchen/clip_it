...
- Windows 11 原生顶层右键菜单写入当前用户注册表，无需管理员权限。
...
- Apple 风格跨平台应用图标；设置页采用故障艺术信号拓扑，以可拖拽不规则实体
  实时呈现局域网设备与连接状态。
- 每台设备可自定义 Emoji 节点图标和显示名称，并通过局域网发现同步展示。
- 托盘自动检查 GitHub Release；更新清单经 Ed25519 验签，安装包再校验
  SHA-256 和大小后自动替换并重启。
...
快捷方式，同时生成供自动更新使用的 App ZIP。把应用拖到 Applications 后双击，
程序会自动安装 Finder 右键快速操作、启用登录启动并进入菜单栏，无需执行终端命令。

Windows 可在 PowerShell 中运行 `./scripts/package-windows.ps1`，生成独立 EXE、ZIP
和 SHA-256 校验文件。所有安装包都在 `dist/` 下；未配置代码签名的构建为未签名
版本，macOS 首次启动时需要右键应用并选择“打开”。配置平台证书后，打包脚本会在
归档前签名并验证应用或 EXE。
...
DMG/App ZIP，并保存为 Actions artifacts。推送与 `Cargo.toml` 版本对应的标签会生成
Ed25519 签名更新清单并自动创建 GitHub Release：
...
Windows Authenticode 签名使用：

- `WINDOWS_CERTIFICATE_BASE64`：代码签名 `.pfx` 的 Base64 内容
- `WINDOWS_CERTIFICATE_PASSWORD`：`.pfx` 密码

标签发布还要求 `UPDATE_SIGNING_PRIVATE_KEY` 为 Ed25519 PKCS#8 PEM。客户端只内置
对应公钥；私钥不得提交到仓库。本仓库的专用私钥已配置为 GitHub Actions Secret。
...
开关，并实时展示局域网设备。在线设备以持续扰动的不规则实体出现；拖入本机核心
会经过“实体 → ASCII 电流/波纹 → 实体”的转换并建立双向连接，拖出时以相反过程
同步断开双方连接。连接关系在双方设备中保存 7 天，
期间设备离线后再次上线会自动连接；到期后需重新拖动连接。端口更新后托盘会重启
后台服务。

### 自动更新

托盘/菜单栏启动后会在后台检查最新 GitHub Release。有新版本时，菜单中的“检查
更新”会变为“安装更新 vX.Y.Z”；点击后下载、验证、替换当前程序并自动重启。也可
在终端中检查或安装：

```powershell
clip-it update check
clip-it update install
```

更新流程先验证 Ed25519 签名清单，只接受本项目 GitHub Release 地址，再核对文件
大小和 SHA-256。Windows 更新独立 EXE，macOS 更新整个 `ClipIt.app`，替换失败会
保留或恢复原应用。
...
- Apple 风格应用图标与故障艺术设备拓扑，支持鼠标牵引、ASCII 电流传导和波纹
  重构动画。
- 断点续传、32 MiB 分块、4 流并发传输和 10/25 GbE 基准工具。
- Ed25519 签名更新、SHA-256 安装包验证、托盘一键自动更新与平台代码签名流水线。
- Windows 11 `IExplorerCommand` 原生顶层菜单扩展，支持多选文件和目录。
